...
export const getAllFlavors = async (req, res) => {
  try {
    ...
    // Agregar el nombre de la categoría a cada sabor
    const flavorsWithCategories = data.map(flavor => {
      const category = categories?.find(cat => cat.id === flavor.category_id);
      return {
        ...flavor,
    ...
    res.json(flavorsWithCategories);
  } catch (error) {
